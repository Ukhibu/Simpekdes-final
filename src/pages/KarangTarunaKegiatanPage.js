...
// Konstanta & Utils
import { DESA_LIST, KARANG_TARUNA_CONFIG } from '../utils/constants';
import { generateKarangTarunaProgramPDF } from '../utils/generateKarangTarunaProgramPDF';
...
// Daftar Bidang Kegiatan Karang Taruna
const BIDANG_KEGIATAN_KT = [
...
const KarangTarunaKegiatanPage = () => {
    ...
    useEffect(() => {
        setLoading(true);
        // Koleksi khusus untuk program kerja Karang Taruna
        const collectionName = 'karang_taruna_program'; 
    ...
    const filteredData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            data = data.filter(item => 
                ...
                (item.bidang || '').toLowerCase().includes(lower) ||
    ...
    }, [searchParams, dataList, currentUser.role, hasOpenedModalFromQuery]);
    ...
    const handleOpenModal = (mode, item = null) => {
        ...
        setFormData(item ? { ...item } : { desa: initialDesa, realisasi: 0, bidang: '' });
        setIsModalOpen(true);
    };
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            if (selectedItem) {
                ...
                showNotification('Program Karang Taruna diperbarui!', 'success');
            } else {
                const docRef = await addDoc(collection(db, collectionName), formData);
                savedId = docRef.id;
                showNotification('Program Karang Taruna ditambahkan!', 'success');
            }
            
            if (currentUser.role === 'admin_desa') {
                const action = selectedItem ? 'memperbarui' : 'menambahkan';
                await createNotificationForAdmins(
                    `Admin Desa ${currentUser.desa} ${action} Program KT: "${formData.nama_program}"`, 
                    `/app/karang-taruna/kegiatan?view=${savedId}`, 
                    currentUser
                );
            }
            handleCloseModal();
    ...
    const handleExportPDF = async () => {
        ...
        try {
            ...
            // 1. Cari Ketua Karang Taruna di desa tersebut
            const ktCollectionName = KARANG_TARUNA_CONFIG?.collectionName || 'karang_taruna';
            const q = query(
                collection(db, ktCollectionName), 
            ...
            if (!querySnapshot.empty) {
                ...
                // Fallback search string "Ketua"
                const qAll = query(collection(db, ktCollectionName), where('desa', '==', effectiveDesa));
            ...
            // 3. Generate PDF
            generateKarangTarunaProgramPDF(filteredData, effectiveDesa, ketuaName, logoUrl);
        ...
        <div className="space-y-6 pb-24">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
                            <FiClipboard className="text-sky-500"/> Program Kerja Karang Taruna
                            {isSelectionMode && <span className="text-sm font-normal px-3 py-1 bg-sky-100 text-sky-700 rounded-full">{selectedIds.length} dipilih</span>}
                        </h1>
                    </div>
                        ...
                        <thead className="bg-gray-50 dark:bg-gray-900/50 text-gray-600 dark:text-gray-300 uppercase text-xs font-semibold border-b border-gray-200 dark:border-gray-700">
                            <tr>
                                <th className="px-6 py-4 w-10 text-center">
                                    {isSelectionMode ? (
                                        <button onClick={toggleSelectAll} className="text-gray-500 hover:text-sky-600 transition-colors">
                                            {selectedIds.length === filteredData.length ? <FiCheckSquare size={20} className="text-sky-600"/> : <div className="w-5 h-5 border-2 border-gray-400 rounded mx-auto"></div>}
                                        </button>
                                    ) : 'No'}
                                </th>
                                ...
                                return (
                                    <tr 
                                        ...
                                        className={`group transition-colors select-none cursor-pointer 
                                            ${isSelected ? 'bg-sky-50 dark:bg-sky-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-700/30'}
                                    ...
                                    >
                                        <td className="px-6 py-4 text-center font-medium text-gray-500 dark:text-gray-400">
                                            {isSelectionMode ? (
                                                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center mx-auto transition-all ${isSelected ? 'bg-sky-600 border-sky-600' : 'border-gray-400'}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-xs text-sky-600 dark:text-sky-400 mt-1 font-medium">{item.bidang || 'Umum'}</div>
                                        ...
                                        <td className="px-6 py-4 text-gray-600 dark:text-gray-300">
                                            <div className="flex items-center gap-1 text-xs mb-1 font-medium">
                                                <FiCalendar className="text-sky-400"/> {formatDateIndo(item.waktu_pelaksanaan)}
                                            </div>
            ...
            {isSelectionMode && (
                <div style={{ position: 'fixed', left: `${menuPos.x}px`, top: `${menuPos.y}px`, zIndex: 9999, touchAction: 'none' }} className="flex items-center gap-3 pl-2 pr-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 shadow-2xl rounded-full animate-in fade-in zoom-in duration-200 backdrop-blur-md">
                    <div onMouseDown={startDrag} onTouchStart={startDrag} className="flex items-center gap-2 px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-full cursor-move group"><FiMove className="text-gray-500"/> <span className="font-bold text-sky-600">{selectedIds.length}</span></div>
