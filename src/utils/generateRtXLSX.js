...
export const generateRtXLSX = async (data) => {
    ...
    const worksheet = workbook.addWorksheet(`Data RT Desa ${namaDesa}`);
    ...
    worksheet.getCell('A1').value = `DATA RT DESA ${namaDesa.toUpperCase()}`;
    ...
    // --- Header Tabel ---
    const headerRowDefs = [
        ["NO", "N A M A", "Jenis Kelamin", null, "JABATAN", "TEMPAT, TGL LAHIR", null, "PENDIDIKAN", null, null, null, null, null, null, null, "NO RT", "DUSUN", "DUKUH", "PRIODE", "No. HP / WA"],
        [null, null, 'L', 'P', null, "TEMPAT LAHIR", "TANGGAL LAHIR", 'SD', 'SLTP', 'SLTA', 'D1', 'D2', 'D3', 'S1', 'S2', null, null, null, null, null]
    ];
    ...
    worksheet.mergeCells(`T${headerStartRow}:T${headerStartRow + 1}`);
    ...
    worksheet.mergeCells(`Q${sigRowStart}:T${sigRowStart}`);
    worksheet.getCell(`Q${sigRowStart}`).value = `${namaDesa}, ${new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}`;
    worksheet.mergeCells(`Q${sigRowStart + 1}:T${sigRowStart + 1}`);
    worksheet.getCell(`Q${sigRowStart + 1}`).value = 'Kepala Desa';
    worksheet.mergeCells(`Q${sigRowStart + 5}:T${sigRowStart + 5}`);
    const kadesCell = worksheet.getCell(`Q${sigRowStart + 5}`);
    kadesCell.value = namaKades;
    kadesCell.font = { name: 'Arial', size: 10, bold: true, underline: true };
    
    for(let i of [0, 1, 5]) {
       worksheet.getCell(`Q${sigRowStart + i}`).alignment = { horizontal: 'center' };
    }
    ...
    worksheet.columns = [
        ...
        { width: 4 }, { width: 4 }, { width: 4 }, { width: 8 }, { width: 15 }, { width: 15 },
        { width: 12 }, { width: 15 }
    ];
    ...
    saveAs(blob, `Data_RT_Desa_${namaDesa.replace(/\s/g, '_')}_${currentYear}.xlsx`);
};
