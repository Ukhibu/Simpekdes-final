...
/**
 * Menghasilkan PDF Laporan Program Kerja PKK
 * @param {Array} programs - Daftar program kerja
 * @param {String} desa - Nama desa
 * @param {String} ketuaName - Nama Ketua TP PKK (untuk tanda tangan)
 * @param {String} logoUrl - URL Logo (Base64 atau Link)
 */
export const generatePKKProgramPDF = (programs, desa, ketuaName, logoUrl = null) => {
  ...
  doc.setFontSize(16);
  doc.text(`TIM PENGGERAK PKK DESA ${desa.toUpperCase()}`, 148.5, 30, { align: 'center' });
  ...
  // --- JUDUL DOKUMEN ---
  doc.setFontSize(12);
  doc.text(`LAPORAN PROGRAM KERJA PKK TAHUN ${currentYear}`, 148.5, 45, { align: 'center' });
  ...
  programs.forEach((program, index) => {
    const rowData = [
      ...
      program.kategori || '-', // Kolom khusus PKK
  ...
  doc.text(`Ketua TP PKK Desa ${desa}`, rightX, finalY + 7, { align: 'center' });
  ...
  doc.save(`Laporan_Program_PKK_${desa}_${currentYear}.pdf`);
};